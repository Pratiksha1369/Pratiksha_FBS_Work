class Furniture {
	int id;
	String name;
	double price;
	static int count = 0;

	// default constructor
	Furniture() {
		System.out.println("Furniture default constructor");
		id = 0;
		name = "Not Given";
		price = 1000;
		count++;
	}

	// parameterized constructor
	Furniture(int id, String name, double price) {
		System.out.println("Furniture parameterized constructor");
		this.id = id;
		this.name = name;
		this.price = price;
		count++;
	}

	int getId() {
		return id;
	}

	void setId(int id) {
		this.id = id;
	}

	String getName() {
		return name;
	}

	void setName(String name) {
		this.name = name;
	}

	double getPrice() {
		return price;
	}

	void setPrice(double price) {
		this.price = price;
	}

	static int getCount() {
		return count;
	}

	static void setCount(int count) {
		Furniture.count = count;
	}

	void display() {
		System.out.println("Furniture ID: " + this.id);
		System.out.println("Furniture Name: " + this.name);
		System.out.println("Price: " + this.price);
	}
}// class Furniture ends here


class Chair extends Furniture {
	String material;
	boolean hasArmRest;

	Chair() {
		super();
		material = "Wood";
		hasArmRest = true;
		System.out.println("Chair default constructor");
	}

	Chair(int id, String name, double price, String material, boolean hasArmRest) {
		super(id, name, price);
		this.material = material;
		this.hasArmRest = hasArmRest;
		System.out.println("Chair parameterized constructor");
	}

	String getMaterial() {
		return material;
	}

	void setMaterial(String material) {
		this.material = material;
	}

	boolean getHasArmRest() {
		return hasArmRest;
	}

	void setHasArmRest(boolean hasArmRest) {
		this.hasArmRest = hasArmRest;
	}

	void display() {
		super.display();
		System.out.println("Material: " + this.material);
		System.out.println("Has Arm Rest: " + this.hasArmRest);
	}
}// class Chair ends here


class Table extends Furniture {
	String shape;
	int noOfLegs;

	Table() {
		super();
		shape = "Rectangle";
		noOfLegs = 4;
		System.out.println("Table default constructor");
	}

	Table(int id, String name, double price, String shape, int noOfLegs) {
		super(id, name, price);
		this.shape = shape;
		this.noOfLegs = noOfLegs;
		System.out.println("Table parameterized constructor");
	}

	String getShape() {
		return shape;
	}

	void setShape(String shape) {
		this.shape = shape;
	}

	int getNoOfLegs() {
		return noOfLegs;
	}

	void setNoOfLegs(int noOfLegs) {
		this.noOfLegs = noOfLegs;
	}

	void display() {
		super.display();
		System.out.println("Shape: " + this.shape);
		System.out.println("No. of Legs: " + this.noOfLegs);
	}
}// class Table ends here


class Bed extends Furniture {
	String size;
	boolean hasStorage;

	Bed() {
		super();
		size = "Queen";
		hasStorage = false;
		System.out.println("Bed default constructor");
	}

	Bed(int id, String name, double price, String size, boolean hasStorage) {
		super(id, name, price);
		this.size = size;
		this.hasStorage = hasStorage;
		System.out.println("Bed parameterized constructor");
	}

	String getSize() {
		return size;
	}

	void setSize(String size) {
		this.size = size;
	}

	boolean getHasStorage() {
		return hasStorage;
	}

	void setHasStorage(boolean hasStorage) {
		this.hasStorage = hasStorage;
	}

	void display() {
		super.display();
		System.out.println("Size: " + this.size);
		System.out.println("Has Storage: " + this.hasStorage);
	}
}// class Bed ends here


class FurnitureInheritance {
	public static void main(String[] args) {

		Chair c1 = new Chair();
		c1.display();
		System.out.println("\n");

		Chair c2 = new Chair(101, "Office Chair", 4500, "Plastic", true);
		c2.display();

		System.out.println("\n\n");

		Table t1 = new Table();
		t1.display();
		System.out.println("\n");

		Table t2 = new Table(202, "Dining Table", 12000, "Round", 4);
		t2.display();

		System.out.println("\n\n");

		Bed b1 = new Bed();
		b1.display();
		System.out.println("\n");

		Bed b2 = new Bed(303, "King Bed", 25000, "King", true);
		b2.display();

		System.out.println("\nTotal Furniture Items: " + Furniture.getCount());
	}
}
